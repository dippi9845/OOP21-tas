...
public interface TeslaFactory {
	...
	static public Tower basicTesla(final Position pos, final List<Enemy> enemyList) {
		final TowerBuilder t = new TowerBuilder(pos, DefaultTowersUtils.TOWERSTOJSONOBJECTMAP.get(DefaultTowers.BASICTESLA), enemyList);
	...
	static public Tower superTesla(final Position pos, final List<Enemy> enemyList) {
		final TowerBuilder t = new TowerBuilder(pos, DefaultTowersUtils.TOWERSTOJSONOBJECTMAP.get(DefaultTowers.SUPERTESLA), enemyList);
		return t.attackType(AttackType.AREA)
				.damageRange(t.getRange())
				.setUpgradable(true)
				.upgradeCost(x -> x/10)
	...
	static public Tower godTesla(final Position pos, final List<Enemy> enemyList) {
		final TowerBuilder t = new TowerBuilder(pos, DefaultTowersUtils.TOWERSTOJSONOBJECTMAP.get(DefaultTowers.GODTESLA), enemyList);
		return t.attackType(AttackType.AREA)
				.damageRange(t.getRange())
				.setUpgradable(true)
				.upgradeCost(x -> x/10)
